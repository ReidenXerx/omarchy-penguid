...
// PenguID: Omarchy's polkit dialog (MIT, see NOTICE) with a face lane. pam_penguid.so asks "Press Enter for face, or
// type your password"; the dialog words each step, and PenguID's face HUD draws the mark above everything.
import QtQuick
import Quickshell
...
Item {
  ...
  // The face lane: offered while PAM shows the Enter prompt, looking after an empty answer to it, refused when PAM
  // asks for the password instead.
  property bool faceOffered: false
  property bool faceLooking: false
  ...
  function syncFromFlow() {
    ...
    if (responseRequired) {
      submitted = false
      var offers = PolkitModel.promptOffersFace(currentPrompt)
      // A password prompt right after an empty answer to the Enter prompt means face did not match.
      if (faceLooking && !offers) faceRefused = true
      if (offers) faceRefused = false
      faceLooking = false
      faceOffered = offers
    }
  }
  ...
  function submitResponse() {
    ...
    submitted = true
    errorFlash = false
    faceLooking = faceOffered && passwordInput.text.length === 0
  ...
  PolkitAgent {
    ...
    onIsRegisteredChanged: {
      if (isRegistered) console.log("penguid polkit agent registered")
      else console.warn("penguid polkit agent is not registered; another agent may be running")
    }
  }
  ...
  PanelWindow {
      ...
      Row {
        ...
        Item {
          ...
          Text {
            ...
            text: root.errorFlash ? "Wrong" : (root.faceLooking ? "Looking for you..." : (root.submitted ? "Checking..." : (root.faceOffered ? "Press Enter for face" : (root.faceRefused ? "Type your password" : "Enter password"))))
            color: root.errorFlash ? Color.polkit.textError : root.foreground
    ...
    // PenguID: PAM's notes under the card, such as irlume's "look at the camera" or why face is paused.
    Rectangle {
      visible: root.dialogVisible && root.currentSupplementary.length > 0
      width: Math.min(supplementaryText.implicitWidth + Style.space(24), panel.width - Style.gapsOut * 2)
      ...
      Text {
        ...
        color: root.supplementaryIsError ? Color.polkit.textError : root.foreground
        font.family: root.fontFamily
